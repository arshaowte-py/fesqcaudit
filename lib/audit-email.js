...
function renderSectionTable(sId, audit) {
  ...
  return `
    <h3 style="margin:20px 0 8px;font-size:15px;color:#111">${escapeHtml(sId)} — ${escapeHtml(SECTION_NAMES[sId])} (${score != null ? score + '%' : '—'})</h3>
...
<body style="font-family:Segoe UI,Arial,sans-serif;color:#111;max-width:900px;margin:0 auto;padding:16px">
  ...
  <table style="width:100%;font-size:14px;margin-bottom:20px">
    ...
    <tr><td style="padding:4px 0;color:#6b7280">Total score</td><td><strong>${audit.totalScore != null ? audit.totalScore + '%' : '—'}</strong></td></tr>
...
export function buildDeleteEmailContent({ storeName, auditorName, auditeeName, visitDate, timestamp }) {
  const deletedAt = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
  const subject = `⚠️ Audit Deleted: ${storeName || 'Unknown Store'}`;

  const html = `<!DOCTYPE html>
<html>
<body style="font-family:Segoe UI,Arial,sans-serif;color:#111;max-width:600px;margin:0 auto;padding:16px">
  <div style="border-bottom:3px solid #FFD400;padding-bottom:12px;margin-bottom:16px">
    <h1 style="margin:0;font-size:20px;color:#dc2626">⚠️ Audit Response Deleted</h1>
    <p style="margin:6px 0 0;color:#6b7280;font-size:14px">Automated alert · ${deletedAt} IST</p>
  </div>
  <p style="font-size:14px;margin:0 0 16px">An audit response has been permanently deleted from the Frido QC Portal.</p>
  <table style="width:100%;font-size:14px;border-collapse:collapse;margin-bottom:20px">
    <tr style="background:#fef2f2">
      <td style="padding:10px 12px;color:#6b7280;width:130px;border:1px solid #fecaca">Store</td>
      <td style="padding:10px 12px;font-weight:700;border:1px solid #fecaca">${escapeHtml(storeName || '—')}</td>
    </tr>
    <tr>
      <td style="padding:10px 12px;color:#6b7280;border:1px solid #fecaca">Auditor</td>
      <td style="padding:10px 12px;border:1px solid #fecaca">${escapeHtml(auditorName || '—')}</td>
    </tr>
    <tr style="background:#fafafa">
      <td style="padding:10px 12px;color:#6b7280;border:1px solid #fecaca">Auditee</td>
      <td style="padding:10px 12px;border:1px solid #fecaca">${escapeHtml(auditeeName || '—')}</td>
    </tr>
    <tr>
      <td style="padding:10px 12px;color:#6b7280;border:1px solid #fecaca">Visit Date</td>
      <td style="padding:10px 12px;border:1px solid #fecaca">${escapeHtml(formatVisitDate(visitDate) || '—')}</td>
    </tr>
    <tr style="background:#fafafa">
      <td style="padding:10px 12px;color:#6b7280;border:1px solid #fecaca">Deleted At</td>
      <td style="padding:10px 12px;border:1px solid #fecaca">${deletedAt} IST</td>
    </tr>
  </table>
  <p style="margin-top:24px;font-size:12px;color:#9ca3af">Sent from Frido QC Audit · ${new Date().toISOString()}</p>
</body>
